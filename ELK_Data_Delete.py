...
def count_documents(elk_host,elk_port,elk_index, field_name, values_chunk):
    url = f"http://{elk_host}:{elk_port}/{elk_index}/_count"
    query = {
        "query": {
            "terms": {
...
def delete_data(elk_host,elk_port,elk_index,query_field,query_field_value_list, timeout="2m"):
    print("DELETING DATA FROM ES.............")
    # ES URL
    es_url = f"http://{elk_host}:{elk_port}/{elk_index}/_delete_by_query?timeout={timeout}"
    ...
    delete_query = {
        "query": {
            "bool": {
                ...
                {
                    "terms": {
                    f"{query_field.upper()}.keyword": query_field_value_list
                    }
                }
                ]
            }
    ...
    response = requests.post(es_url,headers={"Content-Type": "application/json"},data=json.dumps(delete_query))
    print(response)
    # Check the response status
    if response.status_code == 200:
        results = response.json()
        print("Deleted documents:", results.get("deleted", 0))
    else:
...
def main(index_name,query_field):
    ...
    for i in range(0, len(query_field_value_list), batch_size):
        ...
        delete_data(os.getenv('elk_host'),os.getenv('elk_port'),index_name,query_field,batch_list)
